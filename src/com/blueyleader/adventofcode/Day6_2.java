...
public class Day6_2 {
	...
	public static void main(String[] args) {
		...
		int index0,index,index2,index3;
		int opp;
		int x1,x2,y1,y2;
		int total=0;
		String cur;
		...
		while(flag){
			...
			cur= input.substring(0,index0);
			if(cur.indexOf("turn off")!=-1){
				opp=0;
				index=9; 
			}
			else if(cur.indexOf("turn on")!=-1){
				opp=1;
				index=8;
			}
			else{
			...
			index2=cur.indexOf(',');
			x1=Integer.parseInt(cur.substring(index,index2));
			y1=Integer.parseInt(cur.substring(index2+1,cur.indexOf(' ',index2)));
			index = cur.indexOf("through")+8;
			index2=cur.indexOf(',',index);
			x2=Integer.parseInt(cur.substring(index,index2));
			y2=Integer.parseInt(cur.substring(index2+1,index0));
			...
			if(flag){
				input=input.substring(index0+1);
			}
		}
